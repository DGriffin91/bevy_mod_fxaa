...
impl Node for FXAANode {
    ...
    fn run(
        ...
        let (target, fxaa_texture, fxaa) = match self.query.get_manual(world, view_entity) {
            Ok(result) => result,
            Err(_) => return Ok(()),
        };
        ...
        // main_texture -> fxaa_texture
        // TODO convert to ldr with https://gpuopen.com/learn/optimized-reversible-tonemapper-for-resolve/
        {
            let pipeline =
                match pipeline_cache.get_render_pipeline(fxaa_pipeline.to_ldr_pipeline_id) {
        ...
        // fxaa_texture -> main_texture
        {
            let pipeline = match pipeline_cache.get_render_pipeline(fxaa_pipeline.fxaa_pipeline_id)
            {
                Some(pipeline) => pipeline,
                None => return Ok(()),
            };
            ...
            let mut cached_bind_group = self.cached_texture_bind_group.lock().unwrap();
            let bind_group = match &mut *cached_bind_group {
                Some((id, bind_group)) if fxaa_texture.id() == *id => bind_group,
                cached_bind_group => {
                    let sampler = render_context
                        .render_device
                        .create_sampler(&SamplerDescriptor {
                        ...
                        render_context
                            ...
                            .create_bind_group(&BindGroupDescriptor {
                                ...
                                entries: &[
                                    BindGroupEntry {
                                        binding: 0,
                                        resource: BindingResource::TextureView(fxaa_texture),
                                    },
                                    BindGroupEntry {
                                        binding: 1,
                                        resource: BindingResource::Sampler(&sampler),
                    ...
                    let (_, bind_group) = cached_bind_group.insert((fxaa_texture.id(), bind_group));
                    bind_group
                }
            };
